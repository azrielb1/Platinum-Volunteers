...
export function FeaturedEventCard(props) {
    ...
    async function handleSetNotFeatured() {
        let eventid = 0;
        for (let i = 0; i < props.header.length; i++) { eventid += (5 * props.header.charCodeAt(i)) }
        let res = await APIFuncs.setNotFeatured(eventid)
    ...
    let modalActions;
      ...
      if (loadingState === 2) {
        ...
        <div class="two ui buttons">
          <Button color='teal' content='This Event is no longer featured' disabled />
          <Button color='red' onClick={handleRemove} loading={loadingState === 1} >Remove Event</Button>
        </div>
      } else {
          modalActions =
          <div class="two ui buttons">
            <Button color='teal' onClick={handleSetNotFeatured} loading={loadingState === 1} >Remove as Featured Event</Button>
            <Button color='red' onClick={handleRemove} loading={loadingState === 1} >Remove Event</Button>
          </div>
      }
